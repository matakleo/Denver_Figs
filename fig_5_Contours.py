...
cmap=plt.get_cmap(
    'twilight_shifted')
# Create a figure
fig, ax = plt.subplots(nrows=5, ncols=3, figsize=(8.5,10.7),subplot_kw={'projection': crs.PlateCarree()},dpi=350)
        ...
        for TM in TMS:
            ...
            for PBL in PBLS:
                for CL in CLS: 
                    ...
                    ax[row,col].add_feature(cfeature.LAND)
                    ax[row,col].add_feature(cfeature.OCEAN)
...
if CLS[0] == 'xkzm_0.2':
    ...
    ax[0,0].set_title(r'$C_{ k }$\_0.2\_'+PBLS[0], size=size)
    ax[0,1].set_title('Default Case '+PBLS[0],size=size)
