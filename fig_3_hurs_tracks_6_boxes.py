...
legend_names=['Real Track','Default Case',r'$K_{ m }\_lvl_{ 4 }$\_'+PBLS[1],r'$K_{ m }\_lvl_{ 6 }$\_'+PBLS[1]]
if show == 'xkzm':
    fig_name='_xkzm_'
    colors = ['grey', 'blue', 'red']
    CLS=['1.0','km_0.20','km_5.0']
    legend_names=['Real Track','Default Case',r'$C_{ k }$\_0.2\_'+PBLS[1],r'$C_{ k }$\_5.0\_'+PBLS[1]]
    if PBLS[0]=='YSU':
...
fig, ax = plt.subplots(nrows=4, ncols=3, figsize=(10.8,9.8),subplot_kw={'projection': ccrs.PlateCarree()})
    ...
    for HN in HNS :
        ...
        for CL in CLS:
                ...
                ax[row_count,col_count].plot(Eye_Longs[0:len(Eye_Lats)], Eye_Lats[0:len(Eye_Lats)], color=colors[cls_counter],  marker='.', 
                    linewidth='1.7',markersize='8', transform=ccrs.PlateCarree(), label= (CLS[cls_counter]+PBL),zorder=plot_order[cls_counter])
                ax[row_count,col_count].tick_params(axis='x', labelsize=size, length=10, direction = 'in', width = 2)
        ...
        if (col_count==2 and row_count==1):
            ...
            legend_names1=['','',r'$C_{ k }$\_0.2\_'+PBLS[0],r'$C_{ k }$\_5.0\_'+PBLS[0]]
            if show =='lvls':
                legend_names1=['','',r'$K_{ m }\_lvl_{ 4 }$\_'+PBLS[0],r'$K_{ m }\_lvl_{ 6 }$\_'+PBLS[0]]
            ax[1,2].axis("off")
            plt.rc('legend',fontsize=size)
        ...
        if col_count == 3:
            col_count =0
            row_count += 1
h, l = ax[3,1].get_legend_handles_labels()
...
plt.show()
# print('saved as: fig3_tracks'+fig_name+'.png')
# plt.savefig('/Users/lmatak/Desktop/leo_python_scripts/Paper_Figs/figs_saved/fig3_tracks'+fig_name+'.png',bbox_inches='tight')